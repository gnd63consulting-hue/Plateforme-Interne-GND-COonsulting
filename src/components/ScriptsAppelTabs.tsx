...
export default function ScriptsAppelTabs() {
  ...
  return (
    <div>
      {/* Tabs nav */}
      <div className="mb-6 flex flex-wrap gap-1 rounded-2xl border border-gnd-bronze/10 bg-gnd-cream/60 p-1.5">
        {SCRIPTS.map((s) => (
          <button
            key={s.id}
          ...
          >
            {active === s.id && (
              <motion.span
                ...
                className="absolute inset-0 rounded-xl bg-gnd-bronze shadow-warm"
                transition={{ type: 'spring', stiffness: 380, damping: 32 }}
              />
            )}
            <span
              className={`relative z-10 flex items-center gap-1.5 ${
                active === s.id ? 'text-gnd-cream' : 'text-gnd-bronze-soft'
              }`}
            >
      ...
      <div className="mb-5 flex flex-wrap items-baseline gap-x-4 gap-y-1">
        ...
        <span className="font-mono text-[10px] uppercase tracking-[0.18em] text-gnd-amber-dim">
          {current.duration}
        </span>
      </div>
      <p className="mb-7 max-w-2xl text-sm leading-relaxed text-gnd-bronze-soft">
        {current.summary}
      </p>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          {current.warning && (
            <div className="rounded-2xl border border-gnd-amber/30 bg-gnd-amber-pale/40 p-5">
              <div className="mb-2 flex items-center gap-2 font-display text-sm font-medium text-gnd-bronze">
                <AlertTriangle className="h-4 w-4 text-gnd-amber-dim" aria-hidden />
                <span>À NE PAS faire</span>
              </div>
              <ul className="space-y-1.5 text-sm text-gnd-bronze-soft">
                {current.warning.map((w, i) => (
                  <li key={i} className="flex gap-2.5">
                    <span aria-hidden className="mt-2 h-1 w-1 shrink-0 rounded-full bg-gnd-amber-dim" />
                    <span>{w}</span>
                  </li>
                ))}
              </ul>
          ...
          {current.collect && (
            <div className="rounded-2xl border border-gnd-bronze/10 bg-gnd-paper p-5">
              <div className="mb-3 flex items-center gap-2 font-display text-sm font-medium text-gnd-bronze">
                <ClipboardList className="h-4 w-4 text-gnd-amber-dim" aria-hidden />
                <span>Infos à collecter quand le prospect dit OUI</span>
              </div>
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                {current.collect.map((c) => (
                  <div key={c.label}>
                    <p className="mb-1 font-mono text-[9px] font-semibold uppercase tracking-[0.18em] text-gnd-amber-dim">
                      {c.label}
                    </p>
          ...
          {current.binome && (
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div className="rounded-2xl border border-gnd-bronze/10 bg-gnd-paper p-5">
                <p className="mb-2 font-mono text-[9px] font-semibold uppercase tracking-[0.18em] text-gnd-amber-dim">
                  Commercial
                </p>
              ...
              <div className="rounded-2xl border border-gnd-bronze/15 bg-gradient-to-br from-gnd-bronze to-gnd-ink p-5 text-gnd-cream">
                <p className="mb-2 font-mono text-[9px] font-semibold uppercase tracking-[0.18em] text-gnd-amber">
                  Fondateur
                </p>
                <p className="text-sm leading-relaxed text-gnd-cream/85">
...
function ScriptStep({
  ...
  return (
    <motion.div
      ...
      className="rounded-2xl border border-transparent px-5 py-4 transition-colors hover:border-gnd-bronze/8 hover:bg-gnd-cream/40"
    >
      <div className="mb-2 flex items-center gap-3">
        <span className="font-display text-base font-medium italic leading-none text-gnd-amber">
      ...
      {step.quote && (
        <blockquote className="mb-2 border-l-2 border-gnd-amber pl-4 font-display text-base italic leading-snug text-gnd-bronze">
          « {step.quote} »
        </blockquote>
      )}
      {step.body && (
        <div className="text-sm leading-relaxed text-gnd-bronze-soft">
          {step.body}
        </div>
      )}
    </motion.div>
    ...
    steps: [
      {
        ...
        body: (
          <p>
            <span className="font-semibold text-gnd-amber-dim">Si occupé :</span>{' '}
      ...
      },
      {
        ...
        body: (
          <p>
            ...
            <span className="font-semibold text-gnd-bronze">Non / Jamais considéré</span>{' '}
            · <span className="font-semibold text-gnd-bronze">Oui mais insatisfait</span>{' '}
            · <span className="font-semibold text-gnd-bronze">Oui et satisfait</span>.
      ...
      },
      {
        ...
        body: (
          <p>
            ...
            <span className="italic text-gnd-bronze">
    ...
    steps: [
      {
        ...
        body: (
          <p>
            <span className="font-semibold text-gnd-amber-dim">Si non :</span>{' '}
      ...
      },
      {
        ...
        body: (
          <p>
            <span className="font-semibold text-gnd-amber-dim">Acompte 50 %</span>{' '}
    ...
    steps: [
      {
        ...
        body: (
          <ul className="space-y-1.5">
            <li className="flex gap-2"><span aria-hidden className="mt-2 h-1 w-1 shrink-0 rounded-full bg-gnd-amber" /><span>Nom du commerce et activité.</span></li>
            <li className="flex gap-2"><span aria-hidden className="mt-2 h-1 w-1 shrink-0 rounded-full bg-gnd-amber" /><span>Ce qui a été dit en Appels 1 et 2.</span></li>
            <li className="flex gap-2"><span aria-hidden className="mt-2 h-1 w-1 shrink-0 rounded-full bg-gnd-amber" /><span>Besoins identifiés (vitrine / réservation / e-commerce).</span></li>
            <li className="flex gap-2"><span aria-hidden className="mt-2 h-1 w-1 shrink-0 rounded-full bg-gnd-amber" /><span>Point de blocage précis.</span></li>
            <li className="flex gap-2"><span aria-hidden className="mt-2 h-1 w-1 shrink-0 rounded-full bg-gnd-amber" /><span>Personnalité du client (méfiant, pressé, indécis).</span></li>
            <li className="flex gap-2"><span aria-hidden className="mt-2 h-1 w-1 shrink-0 rounded-full bg-gnd-amber" /><span>Budget évoqué si mentionné.</span></li>
          </ul>
        ),
      },
      {
